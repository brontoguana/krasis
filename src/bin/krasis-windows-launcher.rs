...
fn validate_launcher_name(executable: &Path) -> Result<(), String> {
    let stem = executable
        .file_stem()
...
fn main() {
    match run() {
        ...
        Err(message) => {
            ...
            eprintln!(
                "\nDownload the latest Krasis installer or reinstall Krasis to repair the private runtime."
            );
            pause_after_failure();
            std::process::exit(1);
        }
...
mod tests {
    ...
    #[test]
    fn only_the_installed_and_build_launcher_names_are_accepted() {
        assert!(validate_launcher_name(Path::new("Krasis.exe")).is_ok());
        assert!(validate_launcher_name(Path::new("krasis-windows-launcher.exe")).is_ok());
        assert!(validate_launcher_name(Path::new("renamed.exe")).is_err());
    }
