...
Krasis has to make some tradeoffs to make this happen, it stores two copies of the model in System RAM, this allows it to maximise GPU prefill speed and CPU speed,     
  ...
  even on a 16GB GPU, Krasis allows you to selectively quantize differnt components of the model and gives recommendations about the imapact of doing so.  You can         
  ...
  quality or decrease system RAM to get the model to fit.  Krasis always needs to be given the native BF16 HuggingFace model.  It uses this to build the GPU-optimised     
