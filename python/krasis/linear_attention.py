...
class GatedDeltaNetAttention:
    ...
    def __init__(
        ...
        # Split-prefill state on GPU 0 (half-sized, lazy-initialized)
        self._prefill_conv_state: Optional[torch.Tensor] = None
        self._prefill_recurrent_state: Optional[torch.Tensor] = None
    ...
    def reset_state(self):
        ...
        self._prefill_conv_state = None
        self._prefill_recurrent_state = None
    ...
    def _init_prefill_state(self, batch_size: int = 1):
        """Initialize half-sized conv and recurrent states for split prefill on GPU 0."""
        pf_conv_dim = self._prefill_conv_dim
        pf_nv = self._prefill_nv
        if self._prefill_conv_state is None:
            self._prefill_conv_state = torch.zeros(
                batch_size, pf_conv_dim, self.kernel_dim,
                dtype=torch.bfloat16, device=self.device,
            )
        if self._prefill_recurrent_state is None:
            self._prefill_recurrent_state = torch.zeros(
                batch_size, pf_nv, self.k_head_dim, self.v_head_dim,
                dtype=torch.float32, device=self.device,
            )
    ...
    def forward_split(
        ...
        """Half-head-group split forward for tensor parallelism (no out_proj).

        Uses _split_half attribute:
          - half=0 (GPU 0): prefill weight views (first half), half-sized state
          - half=1 (GPU 1): weights pre-split to second half of head groups

        Returns:
            [M, split_nv * v_head_dim] BF16 — no out_proj applied.
        """
        split_half = getattr(self, '_split_half', 1)

        if split_half == 0:
            # GPU 0: use prefill weight views (first half of heads)
            self._init_prefill_state()
            if is_decode:
                return self._forward_split_gpu0_recurrent(hidden)
            else:
                return self._forward_split_gpu0_chunked(hidden)
        else:
            # GPU 1: pre-split weights — use directly
            self._init_state()
            if is_decode:
                return self._forward_split_recurrent(hidden)
            else:
                return self._forward_split_chunked(hidden)

    def _forward_split_gpu0_recurrent(self, hidden: torch.Tensor) -> torch.Tensor:
        """Half-head recurrent decode for GPU 0 using prefill weight views."""
        M = hidden.shape[0]
        nk = self._prefill_nk
        nv = self._prefill_nv
        key_dim = self._prefill_key_dim
        value_dim = self._prefill_value_dim

        qkvz = _linear(hidden, self._prefill_qkvz)
        ba = _linear(hidden, self._prefill_ba)
        q, k, v, z, b, a = self._fix_query_key_value_ordering(
            qkvz, ba, num_k_heads=nk, num_v_heads=nv)

        q_flat = q.reshape(M, key_dim)
        k_flat = k.reshape(M, key_dim)
        v_flat = v.reshape(M, value_dim)

        mixed_qkv = torch.cat([q_flat, k_flat, v_flat], dim=-1)
        mixed_qkv = mixed_qkv.unsqueeze(0).transpose(1, 2)

        conv_input = torch.cat([self._prefill_conv_state, mixed_qkv], dim=-1)
        self._prefill_conv_state = conv_input[:, :, -self.kernel_dim:].clone()

        conv_out = F.conv1d(
            conv_input.to(self._prefill_conv1d.dtype),
            self._prefill_conv1d,
            bias=None, padding=0, groups=self._prefill_conv_dim,
        )
        conv_out = F.silu(conv_out[:, :, -M:]).to(hidden.dtype)
        conv_out = conv_out.transpose(1, 2).squeeze(0)

        q_out = conv_out[:, :key_dim].reshape(M, nk, self.k_head_dim)
        k_out = conv_out[:, key_dim:key_dim * 2].reshape(M, nk, self.k_head_dim)
        v_out = conv_out[:, key_dim * 2:].reshape(M, nv, self.v_head_dim)

        beta = torch.sigmoid(b)
        g = -self._prefill_A_log.float().exp() * F.softplus(a.float() + self._prefill_dt_bias)
        ...
        for t in range(M):
            ...
            self._prefill_recurrent_state = self._prefill_recurrent_state * g_t.unsqueeze(-1).unsqueeze(-1)
            kv_mem = (self._prefill_recurrent_state.squeeze(0) * k_t.unsqueeze(-1)).sum(dim=-2)
            delta = (v_t - kv_mem) * beta_t.unsqueeze(-1)
            self._prefill_recurrent_state = self._prefill_recurrent_state + k_t.unsqueeze(-1).unsqueeze(0) * delta.unsqueeze(-2).unsqueeze(0)
            out_t = (self._prefill_recurrent_state.squeeze(0) * q_t.unsqueeze(-1)).sum(dim=-2)
            outputs.append(out_t)

        attn_out = torch.stack(outputs, dim=0)
        attn_out = self._gated_rmsnorm(attn_out, z)
        return attn_out.reshape(M, nv * self.v_head_dim).to(torch.bfloat16)

    def _forward_split_gpu0_chunked(self, hidden: torch.Tensor) -> torch.Tensor:
        """Half-head chunked prefill for GPU 0 using prefill weight views."""
        M = hidden.shape[0]
        chunk_size = 64
        nk = self._prefill_nk
        nv = self._prefill_nv
        ...
        q_all = conv_out[:, :key_dim].reshape(M, nk, self.k_head_dim)
        k_all = conv_out[:, key_dim:key_dim * 2].reshape(M, nk, self.k_head_dim)
        v_all = conv_out[:, key_dim * 2:].reshape(M, nv, self.v_head_dim)

        beta_all = torch.sigmoid(b)
        a_float = a.float()
        g_all = -self._prefill_A_log.float().exp() * F.softplus(a_float + self._prefill_dt_bias)
        ...
        num_chunks = total_len // chunk_size
        q_c = q_4d.reshape(1, nv, num_chunks, chunk_size, self.k_head_dim)
        k_c = k_4d.reshape(1, nv, num_chunks, chunk_size, self.k_head_dim)
        v_c = v_4d.reshape(1, nv, num_chunks, chunk_size, self.v_head_dim)
        k_beta_c = k_beta.reshape(1, nv, num_chunks, chunk_size, self.k_head_dim)
    ...
    def _forward_split_recurrent(self, hidden: torch.Tensor) -> torch.Tensor:
        """Half-head recurrent decode for GPU 1 (pre-split weights)."""
        M = hidden.shape[0]
        ...
        q_flat = q.reshape(M, self.num_k_heads * self.k_head_dim)
        k_flat = k.reshape(M, self.num_k_heads * self.k_head_dim)
        v_flat = v.reshape(M, self.num_v_heads * self.v_head_dim)
        ...
        conv_out = F.conv1d(
            conv_input.to(self.conv1d_weight.dtype),
            self.conv1d_weight,
            bias=None, padding=0, groups=self.conv_dim,
        )
        conv_out = F.silu(conv_out[:, :, -M:]).to(hidden.dtype)
        conv_out = conv_out.transpose(1, 2).squeeze(0)

        key_dim = self.num_k_heads * self.k_head_dim
        q_out = conv_out[:, :key_dim].reshape(M, self.num_k_heads, self.k_head_dim)
        k_out = conv_out[:, key_dim:key_dim * 2].reshape(M, self.num_k_heads, self.k_head_dim)
        v_out = conv_out[:, key_dim * 2:].reshape(M, self.num_v_heads, self.v_head_dim)
    ...
    def _forward_split_chunked(self, hidden: torch.Tensor) -> torch.Tensor:
        """Half-head chunked prefill for GPU 1 (pre-split weights)."""
        M = hidden.shape[0]
        chunk_size = 64
        ...
        key_dim = self.num_k_heads * self.k_head_dim
        value_dim = self.num_v_heads * self.v_head_dim
        ...
        q_all = conv_out[:, :key_dim].reshape(M, self.num_k_heads, self.k_head_dim)
        k_all = conv_out[:, key_dim:key_dim * 2].reshape(M, self.num_k_heads, self.k_head_dim)
        v_all = conv_out[:, key_dim * 2:].reshape(M, self.num_v_heads, self.v_head_dim)

        beta_all = torch.sigmoid(b)
        a_float = a.float()
        g_all = -self.A_log.float().exp() * F.softplus(a_float + self.dt_bias)
