...
class GQAAttention:
    ...
    def forward_split(
        ...
        """Half-head split attention for tensor parallelism (no o_proj).

        Uses _split_half attribute:
          - half=0 (GPU 0): full q_proj, slice output to first half of heads
          - half=1 (GPU 1): q_proj pre-split to second half of heads
        k_proj/v_proj are always full (replicated).

        Returns:
            [M, split_num_heads * head_dim] BF16 (with gated attention applied)
            — no o_proj applied, caller must concat halves and apply o_proj.
        """
        M = hidden.shape[0]
        split_half = getattr(self, '_split_half', 0)

        # ── Q/K/V projections ──
        # GPU 0 uses _prefill_q_proj view (first half of heads) → half FLOPS
        # GPU 1 has pre-split q_proj (second half of heads)
        if split_half == 0:
            q_proj_w = self._prefill_q_proj
            actual_heads = self._prefill_num_heads
        else:
            q_proj_w = self.q_proj
            actual_heads = self.num_heads
        q_raw = _linear(hidden, q_proj_w)
        k = _linear(hidden, self.k_proj)
        v = _linear(hidden, self.v_proj)

        # Gated attention: split q_proj output into query + gate
        if self.gated_attention:
            q_raw = q_raw.view(M, actual_heads, self.head_dim * 2)
            q, attn_gate = q_raw.chunk(2, dim=-1)
            attn_gate = attn_gate.reshape(M, actual_heads * self.head_dim)
        else:
            q = q_raw
        ...
        if not self.gated_attention:
            if self.q_proj_bias is not None:
                bias = getattr(self, '_prefill_q_proj_bias', self.q_proj_bias) if split_half == 0 else self.q_proj_bias
                q = q + bias
        if self.k_proj_bias is not None:
            k = k + self.k_proj_bias
        if self.v_proj_bias is not None:
        ...
        # Reshape to per-head
        if not self.gated_attention:
            q = q.reshape(M, actual_heads, self.head_dim)
        k = k.reshape(M, self.num_kv_heads, self.head_dim)
        v = v.reshape(M, self.num_kv_heads, self.head_dim)
        ...
        k_append = k.to(kv_cache.kv_dtype)
        v_append = v.to(kv_cache.kv_dtype)
        ...
        # FlashInfer paged attention with split Q heads
        kv_indices = seq_state.kv_indices(self.device)
        kv_indptr = seq_state.kv_indptr(self.device)
        effective_kv_len = seq_state.seq_len + num_new_tokens
        kv_len_arr = torch.tensor([effective_kv_len], dtype=torch.int32, device=self.device)
        ...
        if self.sinks is not None:
            attn_out, lse = run_result
            # Use prefill sinks view for GPU 0, pre-split sinks for GPU 1
            if split_half == 0:
                sinks_slice = getattr(self, '_prefill_sinks', self.sinks[:actual_heads])
            else:
                sinks_slice = self.sinks
            sink = sinks_slice.view(1, -1)
            scale = torch.sigmoid(lse - sink)
            attn_out = attn_out * scale.unsqueeze(-1)
        else:
            attn_out = run_result

        # Flatten to [M, actual_heads * head_dim] — no o_proj
        attn_flat = attn_out.reshape(M, actual_heads * self.head_dim)
